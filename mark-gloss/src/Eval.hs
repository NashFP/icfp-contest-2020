module Eval(Value(..), Env, valueToList, valueToCoordinatePair, eval, evaluateProgram, stdlib, repl) where
...
writeBitmaps :: [Value] -> [Value] -> [Value]
writeBitmaps bitmaps blist =
  unsafePerformIO $ do
...
multipleDraw v =
  let bitmaps = map renderBitmap $ valueToList v in
    listToValue $ writeBitmaps bitmaps (valueToList v)
